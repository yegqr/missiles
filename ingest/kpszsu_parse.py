...
# КИЇВСЬКА ПРИВ'ЯЗКА. Задача — відрізнити МІСТО від області, і обидві помилки
# тут коштують дорого й по-різному:
#   * «на Київщині», «Київська область (БЕЗ КИЄВА)», «на Київське водосховище»
#     не є ударом по місту — але підрядок «на Київ» у них є;
#   * «Ракети на Житомирщині змінили курс на Київ!» — це УДАР ПО МІСТУ, хоч у
#     тексті названо чужу область. Відкидати такі повідомлення означає губити
#     саме масовані нальоти, де перелічують кілька напрямків.
# Тому: спершу ВИРІЗАЄМО обласні токени з тексту, і аж потім шукаємо місто.
# Згадка іншої області сама по собі нічого не вирішує.
RE_OBLAST_TOK = re.compile(r"Київщин\w*|Київськ\w*|Київське\s+водосховищ\w*|"
                           r"Київського\s+водосховищ\w*", re.I)
RE_CITY = re.compile(r"\bКиїв\b|\bКиєв[оа]м?\b|\bКиєва\b|\bКиєві\b|\bКиїву\b|"
                     r"на\s+Київ\b|у\s+Київ\b|в\s+Київ\b|над\s+Києвом", re.I)
# Самої назви міста замало: «зброяр «Привидів Києва»» — не супровід цілі.
# Потрібна ознака РУХУ або ЗАГРОЗИ в тому ж повідомленні.
RE_MOVE = re.compile(r"курс|напрям|рух\w*|прямує|леж?ить|летить|"
                     r"в укритт|у укритт|загроз|небезпек|застосуванн|атак", re.I)


def city_target(text: str) -> bool:
    """Чи йдеться про ціль на МІСТО Київ."""
    return bool(RE_CITY.search(RE_OBLAST_TOK.sub(" ", text))) and bool(RE_MOVE.search(text))
...
def run():
    ...
    for msg_id, ts_raw, raw in rows:
        ...
        # Супровід цілей ПО МІСТУ.
        if len(text) < 400 and city_target(text):
