-- 019: лейбл без запасного «false»
--
-- v_daily_labels мала COALESCE(a.attacked, w.drone_tracks_kyiv > 0, false).
-- Останній аргумент перетворював «жодне джерело нічого не сказало» на
-- впевнене «атаки не було» — 75 діб 2022 року з понад трьома годинами
-- тривоги в місті отримували саме його. Прибираємо: невідоме лишається
-- невідомим, а споживач вирішує явно.
...
COMMENT ON VIEW v_daily_labels IS
  ...
  'Це НЕ «атаки не було»: 303 доби 2022-2023 не мають жодного спостереження, '
  'і перетворення їх на нуль робило б розмітку фальшиво повною.';
